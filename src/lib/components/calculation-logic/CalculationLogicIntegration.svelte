<script lang="ts">
    ...
    // Props
    let { fieldId, fieldName, onCalculationLogicSaved, questionCard, questionList } = $props();
    ...
    async function handleSave(calculationData) {
        ...
        if (onCalculationLogicSaved) {
            onCalculationLogicSaved(calculationData);
        }
                ...
                <div class="rounded border border-gray-200 bg-white p-2 text-xs">
                    <div class="flex items-start justify-between gap-2">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-gray-500 font-mono bg-gray-100 p-1 rounded text-xs truncate mt-1">
                                {rule.expression}
                            </div>
